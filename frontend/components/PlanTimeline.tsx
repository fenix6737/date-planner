...
import { cacheSpot, planItemToSpot } from "@/lib/spotCache";
...
export default function PlanTimeline({ items, budget }: PlanTimelineProps) {
  ...
  const handleDetailClick = (item: PlanSpotItem) => {
    cacheSpot(planItemToSpot(item));
  };
    ...
    <div className="glass flex h-full flex-col">
      <div className="border-b border-white/50 px-5 py-4">
        ...
        <div className="mt-3 grid grid-cols-2 gap-3">
          <div className="rounded-xl bg-[#8b5e3c]/10 px-3 py-2">
            <p className="text-xs font-medium text-[#8b7355]">予算</p>
            <p className="text-lg font-bold text-[#5c4030]">{budget.toLocaleString()}円</p>
          </div>
          <div className="rounded-xl bg-[#8b5e3c]/10 px-3 py-2">
            <p className="text-xs font-medium text-[#8b7355]">使う見込み</p>
            <p className="text-lg font-bold text-[#5c4030]">{total.toLocaleString()}円</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 space-y-0 overflow-y-auto p-4">
        {items.map((item, idx) => {
          ...
          const spotId = encodeURIComponent(item.source_id || item.name);
          ...
          return (
            <div key={`${item.name}-${idx}`} className="relative flex gap-4 pb-6">
              ...
              <div className="glass-soft min-w-0 flex-1 p-4">
                ...
                <div className="mt-3 rounded-lg border border-[#8b5e3c]/20 bg-white/60 px-3 py-2">
                  <p className="text-xs font-medium text-[#8b7355]">料金目安</p>
                  <p className="text-base font-bold text-[#5c4030]">
                ...
                {item.address && (
                  <p className="mt-2 text-xs text-[#8b7355]">{item.address}</p>
                )}
                <Link
