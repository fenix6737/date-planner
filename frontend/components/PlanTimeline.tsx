            ...
            <div key={`${item.name}-${idx}`} className="relative flex gap-4 pb-6">
              ...
              <div className="glass-soft min-w-0 flex-1 p-4">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-3 text-sm">
                  <span className="rounded-full bg-[#8b5e3c]/10 px-2.5 py-1 text-sm font-semibold text-[#5c4030]">
                    料金目安: {item.budget_est > 0 ? `約 ${item.budget_est.toLocaleString()}円` : "無料"}
                  </span>
                ...
                <Link
                  href={`/spots/${encodeURIComponent(spotId)}?source=${encodeURIComponent(item.source)}&name=${encodeURIComponent(item.name)}&lat=${item.lat}&lng=${item.lng}`}
